...
!function(){"use strict";angular.module("POS17").config(["$urlRouterProvider","$stateProvider",function(r,e){e.state("main.purchase_order",{url:"/purchaseorder",templateUrl:"modules/purchase_order/purchase_order.html",controller:"purchaseOrderController"}).state("main.add_order",{url:"/purchaseorder/:selected_id",templateUrl:"modules/purchase_order/addor.html",controller:"addOrderController"}).state("main.list",{url:"/purchaseorder/hold",templateUrl:"modules/purchase_order/producthold.html",controller:"holdController"}).state("main.productlist",{url:"/purchaseorder/productList",templateUrl:"modules/purchase_order/productList.html",controller:"productsController"}).state("main.stock",{url:"/purchaseorder/stocks",templateUrl:"modules/purchase_order/stocks.html",controller:"stocksController"}).state("main.po_view",{url:"/purchaseorder/view",templateUrl:"modules/purchase_order/purchase_order-view.html",controller:"purchaseOrderController"})}])}();
!function(){"use strict";function e(e,r,s,t,a,o,c,l){e.puchaseorders=[];var n=JSON.parse(t.get("user"));a.logTransaction("View Purchase Orders","PURCHASE ORDER","ALL","User:="+n.username+" View all list of Inventory - Purchase Order"),e.updatePurchaseOrder=function(e,r){console.log("id",e),console.log("data",r)},e.toggleModal=function(s){l.openConfirm({templateUrl:"modules/dialog/deleteDialog.html",scope:e,className:"ngdialog-theme-default"}).then(function(){r.deletePurchaseOrder(s).then(function(e){return 200==e.statusCode&&e.response.success?(c.success(e.response.msg,"Success"),void o.reload()):(console.log("data.response;",e),void c.error(e.response.msg,"Error"))})},function(e){console.log("Modal promise close: ")})};var i=function(){e.puchaseordersisLoading=!0,e.puchaseorders=[],e.paginate={offset:0,limit:10},e.puchaseordersTable=new s({page:1,count:10},{counts:[10,25,50,100],getData:function(s,t){r.getAllPurchaseOrder(e.paginate).then(function(r){if(200==r.statusCode&&r.response.success){var a=r.response.result;e.txtSearch&&"All"==e.applicationStatus?e.puchaseorders=$filter("filter")(a,e.txtSearch):e.txtSearch&&"Published"==e.applicationStatus?(e.puchaseorders=$filter("filter")(a,{status:1}),e.puchaseorders=$filter("filter")(e.puchaseorders,e.txtSearch)):e.txtSearch&&"Unpublished"==e.applicationStatus?(e.puchaseorders=$filter("filter")(a,{status:0}),e.puchaseorders=$filter("filter")(e.puchaseorders,e.txtSearch)):_.isUndefined(e.txtSearch)&&"All"==e.applicationStatus?e.puchaseorders=a:_.isUndefined(e.txtSearch)&&"Published"==e.applicationStatus?e.puchaseorders=$filter("filter")(a,{status:1}):_.isUndefined(e.txtSearch)&&"Unpublished"==e.applicationStatus?e.puchaseorders=$filter("filter")(a,{status:0}):e.puchaseorders=a,t.total(e.puchaseorders.length);var o=e.puchaseorders.slice((t.page()-1)*t.count(),t.page()*t.count());s.resolve(o),e.puchaseordersisLoading=!1}else c.error(r.response.msg,"Error")})}})};i()}angular.module("POS17").controller("purchaseOrderController",e),e.$inject=["$scope","purchaseOrderFactory","ngTableParams","localStorageService","transLogFctry","$state","toastr","ngDialog"]}();
!function(){"use strict";function e(e,s,t,o,r,a,n,i,l,u,c,d,p,m,f,g,h){e.isColl=!1,e.radioModel="product",e.products=[],e.productBundles=[],e.items={},e.details={},e.suppliers=[],e.customers=[],e.stores=[],e.details.products_list=[],e.details.details=[];var M=!1,v=5e3;e.showMessage=!1,e.showsecondMessage=!1,e.showExportMessage=!1,e.enAbleToSendForm,e.profile={},e.datenow=h(new Date).format("MMMM Do YYYY, h:mm:ss a"),e.doGreeting=function(){M&&i.cancel(M),e.showMessage=!0,M=i(function(){e.showMessage=!1,e.showsecondMessage=!0,M=i(function(){e.showsecondMessage=!1},2e3)},v)},e.doExport=function(){M&&i.cancel(M),e.showExportMessage=!0,M=i(function(){e.showExportMessage=!1,e.showsecondExportMessage=!0,M=i(function(){e.showsecondExportMessage=!1},2e3)},v)},e.changeData=function(s){e.radioModel=s},e.getoveralltotal=function(s){e.overalltotal=_.sumBy(s,function(e){return e.price*e.quantity})},a.selected_id&&m.getAllPurchaseOrderDetail(a.selected_id).then(function(s){s&&200==s.statusCode&&s.response.success&&(e.details=s.response.result,e.getoveralltotal(e.details.details))}),c.getAllSuppliers().then(function(s){s&&200==s.statusCode&&s.response.success&&(e.suppliers=s.response.result)}),p.getAllPerson().then(function(s){s&&200==s.statusCode&&s.response.success&&(e.customers=_.filter(s.response.result,function(e){return 2==e.per_type_id}))}),u.getAllProduct().then(function(s){if(s&&200==s.statusCode&&s.response.success){var t=s.response.result;_.each(t,function(s){0==s.is_archived&&e.products.push(s)})}}),f.getAllStore().then(function(s){s&&200==s.statusCode&&s.response.success&&(e.stores=s.response.result)}),u.getAllProductBundle().then(function(s){if(s&&200==s.statusCode&&s.response.success){var t=s.response.result;_.each(t,function(s){1!==s.is_archived&&e.productBundles.push(s)})}}),e.calculateItem=function(e){var s=0,t=0,t=e.price*e.quantity;return s+=parseFloat(t),e.total=s,e.total},e.getSupplierName=function(s){if(s)return _.find(e.suppliers,function(e){return e.s_id==s})},e.parseDate=function(e){if(e)return h(new Date(e)).format("MMMM Do YYYY, h:mm:ss a")},e.getCurrentUser=function(){s.userprofile().then(function(s){if(200==s.statusCode&&s.response.success)e.profile=s.response.result,e.currentemail=e.profile.profile.email;else if(400==s.statusCode&&!s.response.success)return void l.error(s.response.msg,"Error")})},e.toast_emailnotifs=function(e){"Sent"==e||"sent"==e?l.success("Email Sent","Success"):l.warning("Email not Sent","Warning")},e.currentPage=1,e.entryLimit=5,e.selectDeselectAll=!1,e.applicationStatus="All",e.displayModel="List",e.viewby=25,e.pageSize=e.viewby,e.currentPage=1,e.itemsPerPage=20,e.totalUsers=0,e.addToList=function(s){_.isEmpty(s)?l.warning("Product list could not empty","Warning"):"product"==e.radioModel?_.isUndefined(s.product)||(e.details.details.push({item_name:s.product.item_name,quantity:s.quantity,price:s.price,discount:s.discount,total:s.total,product_id:s.product.p_id,bundle_id:null}),e.getoveralltotal(e.details.details)):"productgroup"==e.radioModel&&(_.isUndefined(s.productgroup)||(e.details.details.push({item_name:s.productgroup.bundle_name,quantity:s.quantity,price:s.price,discount:s.discount,total:s.total,product_id:null,bundle_id:s.productgroup.pb_id}),e.getoveralltotal(e.details.details)))},e.savePO=function(s){var o=JSON.parse(t.get("user"));e.details.created_by=o.uc_id,e.details.products_list=e.details.details,e.details.emailmessage=$("#po1").html(),e.details.toSend=s,e.details.supplier_id&&e.details.deliver_to&&e.details.po_number?a.selected_id?m.updatePurchaseOrder(a.selected_id,e.details).then(function(t){t&&200==t.statusCode&&t.response.success?l.success(t.response.msg,"Success"):l.error(t?t.response.msg:"Error Found","Eror"),s?e.toast_emailnotifs(t&&t.response.email?t.response.email:"Error Found"):""}):m.savePurchaseOrder(e.details).then(function(t){t&&200==t.statusCode&&t.response.success?l.success(t.response.msg,"Success"):l.error(t?t.response.msg:"Error Found","Eror"),s?e.toast_emailnotifs(t&&t.response.email?t.response.email:"Error Found"):""}):l.warning("Fill in all the required fields in the first tab","Warning")},e.cancelPO=function(){var s=JSON.parse(t.get("user"));e.details.created_by=s.uc_id,e.details.products_list=e.details.details,e.details.emailmessage=$("#po1").html(),e.details.po_status="canceled",e.details.supplier_id&&e.details.deliver_to&&e.details.po_number?a.selected_id?m.updatePurchaseOrder(a.selected_id,e.details).then(function(e){e&&200==e.statusCode&&e.response.success?l.success("PO status Cancelled","Success"):l.error(e?e.response.msg:"Error Found","Eror")}):m.savePurchaseOrder(e.details).then(function(e){e&&200==e.statusCode&&e.response.success?l.success("PO status Cancelled","Success"):l.error(e?e.response.msg:"Error Found","Eror")}):l.warning("Fill in all the required fields in the first tab","Warning")},e.toggleModal=function(){g.openConfirm({templateUrl:"modules/dialog/cancelDialog.html",scope:e,className:"ngdialog-theme-default"})},e.calculateItems=function(s){var t=0,o=0;_.each(e.details.products_list,function(e){var s=e.price-e.discount;t=s*e.quantity,o+=parseFloat(t)}),e.details.sub_total=o,e.calculateBilling()},e.calculateBilling=function(){e.total=parseFloat(e.details.sub_total||0)+parseFloat(e.details.tax||0)+parseFloat(e.details.shipping_fee||0)},e.removeProduct=function(s){e.products_list.splice(s,1)},e.edit=function(e){n.go("main.products-add",{p_id:e})},e.openPOUpdateModal=function(s){e.enAbleToSendForm=s,e.details.po_status="",g.openConfirm({templateUrl:"./modules/purchase_order/po.update.modal.html",scope:e,className:"ngdialog-theme-default",appendClassName:"ngdialog-custom",width:"40%"}).then(function(){e.savePO(s)},function(e){console.log("Modal promise close: ")})},e.init=function(){e.getCurrentUser()},e.init()}angular.module("POS17").controller("addOrderController",e),e.$inject=["$scope","AuthFctry","localStorageService","$http","$filter","$stateParams","$state","$timeout","toastr","productFactory","supplierFctry","ngTableParams","humanFactory","purchaseOrderFactory","storeFactory","ngDialog","moment"]}();
