...
!function(){"use strict";function e(e,s,t,r,o,n,i,c){e.prodStores=[],e.selectedStores=[],e.inv.unit={},e.inv.unit.selected={},e.inv.unit.list=[],e.applicationStatus="",e.isloadingstores=!1,e.istrackingloader=!1,e.getAllStore=function(){e.stores=[],e.storesCopy=[],e.stockTrackQty=[],e.stockTrackLevel=[],e.stockTrackUnit=[],e.inv_stockTrackDets=[],async.waterfall([function(s){n.getAllStore().then(function(t){200==t.statusCode&&t.response.success?(e.stores=t.response.result,e.storesCopy=angular.copy(e.stores),s(null,e.stores)):r.error(t.response.msg,"Error")})},function(s,t){_.isEmpty(o.p_id)?e.itemArray=angular.copy(s):i.getPriceHistory(o.p_id).then(function(t){200==t.statusCode&&t.response.success&&t.response.result.length>0?(e.prodStore=t.response.result,_.each(e.storesCopy,function(s){var t=_.find(e.prodStore,function(e){return s.store_id===e.store_id});t&&(s.selected=!0,s.base_price=t.base_price,s.regular_price=t.regular_price,s.rrp=t.rrp,s.sale_price=t.sale_price,s.sale_percent=t.sale_percent,s.fk_product_id=o.p_id,e.selectedStores.push(s))}),e.itemArray=angular.copy(e.storesCopy)):200==t.statusCode&&t.response.success&&0===t.response.result.length?e.itemArray=angular.copy(s):r.error(t.response.msg,"Error")})}])},e.getAllStoresByProduct=function(){_.isEmpty(o.p_id)||c.getProductAndStores(o.p_id).then(function(s){s&&s.response&&s.response.result&&(e.stockTrack=s.response.result[0]&&s.response.result[0].is_enable_tracking?s.response.result[0].is_enable_tracking:0,console.log("le ",s.response.result.length),_.each(s.response.result,function(t,r){var o=s.response.result[r];e.inv_id=o?o.inv_id:null,e.copy_of_inv_id=angular.copy(e.inv_id),e.stockTrackQty[r]=o?o.stock_onhand:0,e.stockTrackLevel[r]=o?o.reorder_level:0;var n={selected:{_id:t.sales_unit_id,label:t.sales_unit}};e.stockTrackUnit[r]=n,s.response.result.length-1==r?(console.log("index if ",r),e.isloadingstores=!1):(e.isloadingstores=!0,console.log("index else ",r))}),e.isloadingstores=!1)})},e._getAllProductUnits=function(){e.inv.unit.list=[],c.getAllProductUnits().then(function(s){if(s&&200==s.statusCode&&s.response.success){if(s&&s.response&&s.response.result)for(var t=s.response.result.length,r=0;r<t;r++){var o={_id:s.response.result[r].iu_id,label:s.response.result[r].unit_name,unit_symbol:s.response.result[r].unit_symbol,selected:!1};e.inv.unit.list.push(o)}}else console.log("at else ",s)})},e.getUnitsByProduct=function(){_.isEmpty(o.p_id)||c.getAllProductUnitsByPID(o.p_id).then(function(e){e&&e.response&&e.response.result?console.log("getUnitsByProduct ar if"):console.log("getUnitsByProduct at else")})},e.paginate=function(){$("#tblStore").trigger("footable_redraw")},e.updateStatus=function(s){e.applicationStatus=s,""===s?e.itemArray=angular.copy(e.storesCopy):(e.itemArray.splice(0,e.itemArray.length),_.each(e.storesCopy,function(s){_.find(e.selectedStores,function(e){return s.store_id===e.store_id})&&e.itemArray.push(s)}))},e.searchStore=function(){if(e.txtSearch){e.itemArray.splice(0,e.itemArray.length);var s=t("filter")(e.storesCopy,e.txtSearch);e.itemArray=angular.copy(s)}},e.searchStatus=function(){""===e.txtSearch&&(e.itemArray.splice(0,e.itemArray.length),e.itemArray=angular.copy(e.storesCopy))},e.changeSelectDeselectAll=function(){_.each(e.itemArray,function(s){s.selected=e.selectDeselectAll,e.checkStoresChanges(s)})},e.checkStoresChanges=function(s){if(s.selected){var t=_.find(e.selectedStores,function(e){return e.store_id===s.store_id});t||(s.base_price=0,s.regular_price=0,s.rrp=0,s.sale_price=0,s.sale_percent=0,s.fk_product_id=o.p_id,e.selectedStores.push(s))}else e.selectedStores=_.filter(e.selectedStores,function(e){return e.store_id!==s.store_id})},e.saveStore=function(s){return o.p_id?void i.saveSP(o.p_id,e.selectedStores).then(function(e){200===e.statusCode&&e.response.success?r.success(e.response.msg,"Success"):r.error(e.response.msg,"Error")}):void r.warning("WARNING: Unable to save.. no available products","WARNING")},e._pushToStockTrack=function(s,t,r,n,i){var c={store_id:t,stockTrackQty:r||0,stockTrackLevel:n||0,p_id:o.p_id,sales_unit_id:i?i.selected._id:null,purchase_unit_id:null,qty_convert:null,inv_id:e.copy_of_inv_id,stockTrack:!0};e.inv_stockTrackDets[s]=c},e.trackThisProduct=function(){e.istrackingloader=!0,c.assignStore(o.p_id,e.inv_stockTrackDets).then(function(s){s&&200===s.statusCode&&s.response.success?(e.istrackingloader=!1,r.success("Success","Success")):(r.error("Fail","Error"),e.istrackingloader=!1)})},e.init=function(){e.isloadingstores=!0,e.getAllStore(),e._getAllProductUnits(),e.getAllStoresByProduct(),e.getUnitsByProduct()},e.init()}angular.module("POS17").controller("productStoreCtrl",e),e.$inject=["$scope","$state","$filter","toastr","$stateParams","storeFactory","priceHistoryFactory","productFactory"]}();
