...
!function(){"use strict";function e(e,t,o,i,s,r,n,a,u,c,l){s.user=_.isEmpty(r.get("User"))?[]:JSON.parse(r.get("User")),e.products=_.isEmpty(r.get("Products"))?[]:JSON.parse(r.get("Products")),e.company=_.isEmpty(r.get("Company"))?[]:JSON.parse(r.get("Company")),e.companyname=s.user.company_name,e.details={},e.radioModel="Capex",u.quote_id&&a.getQuoteDetail(u.quote_id,function(t,o){200==o.statusCode&&o.response.success&&(e.details=o.response.result,e.details.type=e.radioModel,e.details.customer=e.details.bill_qoute_info,e.details.date_created=moment(e.details.date_created).format("YYYY-MM-DD"),e.details.valid_until=moment(e.details.valid_until).format("YYYY-MM-DD"),console.log("$scope.details:",e.details),a.getProductBundle(function(t){e.result=[],200==t.statusCode&t.response.success&&(_.each(e.details.quote_item_list,function(e){_.find(t.response.result,function(t){e.bundle_id==t.pb_id&&(_.each(t.products,function(e){e.category_id=e.product_detail.product_category[0].category_id,e.sell_price=e.product_detail.sell_price}),e.products=t.products)})}),e.items=_.map(e.details.quote_item_list,function(t){return t.products&&_.each(t.products,function(t){e.result.push(t)}),_.each(e.products,function(e){t.bundle_id==e.pb_id&&(t.spprices=e.spPrice)}),t.product_id?t:t}),e.totalupfront=_.sum(_.map(e.items,function(e){return e.price*e.quantity})),e.totalupfrontopex=_.sum(_.map(e.items,function(e){return e.price})),e.opexquantity=_.sum(_.map(e.items,function(e){return e.quantity})),e.subtotalOpex=_.sum(_.map(e.items,function(e){_.isUndefined(e.spprices)&&(e.spprices=0);var t=(31.111*e.price/1e3+e.spprices)*e.quantity;return t})),e.spprice=_.sum(_.map(e.items,function(e){return e.spprices*e.quantity})))}))}),e.$watch("radioModel",function(t){console.log("newValue:",t),l.trackEvent("Quote Generation","generate quote",t,"-",e.details.quote_number,e.details.quote_id)}),e.newvalue=function(){e.details.type=e.radioModel},e.printQoute=function(e){i.print()},e.openShareModal=function(){n.open({animation:!0,templateUrl:"./views/share.modal.html",size:"md",windowClass:"share-modal",controller:"ShareModalInstanceCtrl",resolve:{article:function(){return e.details},param:function(){return"quote"}}})}}angular.module("tessaApp").controller("QuoteCtrl",e),e.$inject=["$scope","sharerFactory","toastr","$window","$rootScope","localStorageService","$uibModal","APIFactory","$stateParams","$filter","$frontierAnalytics"]}();
