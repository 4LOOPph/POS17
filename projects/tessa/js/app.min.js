...
!function(){"use strict";function t(t,e,i,a,r,l,s,c,n,o,u,d,f,g){f.loadImages().then(function(t){t.data.items.forEach(function(t){var e=t.description,i=e.match(/width="(.*?)"/)[1],a=e.match(/height="(.*?)"/)[1];t.actualHeight=a,t.actualWidth=i}),e.pics=t.data.items}),e.refresh=function(){g.gallery.refresh()},e.seemore=!0,e.date=new Date,s.user=r.isEmpty(o.get("User"))?[]:JSON.parse(o.get("User")),e.expFilter=r.isEmpty(o.get("articlefilter"))?"all":JSON.parse(o.get("articlefilter")),e.selectedItem=[],e.articlesList=[],e.lists=[],e.tags=[],e.tagscopy=[],e.Portfolio=[],e.Article=[],e.selectedTag=[],e.tagselected=[],e.infiniteScrollData={},e.infiniteScrollData.numToAdd=1,e.infiniteScrollData.currentItems=10,e.dynamicPopover={templateUrl:"myPopoverTemplate.html"},e.pdf={file:null,thumbnail:""},e.resetInfScroll=function(){e.infiniteScrollData.currentItems=e.infiniteScrollData.numToAdd},e.addMoreItems=function(){e.infiniteScrollData.currentItems!==e.lists.length&&(e.infiniteScrollData.currentItems+=e.infiniteScrollData.numToAdd,e.articlesList=e.lists,e.articlesList.length=e.infiniteScrollData.currentItems)},e.generateArticles=function(){e.articlesList=[],e.tags=[],e.tagscopy=[],e.lists=[],e.selectedTag=[],i.getArticles(function(t,i){if(200==i.statusCode&&i.response.success){r.each(i.response.result,function(t){t.name=t.title,t.type="Experience",t.tags.length>=1&&1==t.is_active&&e.articlesList.push(t)}),e.articlesList=r.map(r.orderBy(e.articlesList,"date_created")).reverse();for(var a=0;a<e.articlesList.length;a++)for(var l=0;l<e.articlesList[l].tags.length;l++)e.articlesList[a].tags[l]&&e.tags.push(e.articlesList[a].tags[l]);r.each(e.articlesList,function(t){if(t.mediafiles.length>0&&(t.image=t.mediafiles[0].media_path),!r.isNull(t.thumbnail_media)){var e=t.article_thumbnail;if(e){if("video/mp4"==e.media_mimetype)return t.article_thumbnail.url="http://www.videogular.com/styles/themes/default/latest/videogular.css";if("video/x-flv"==e.media_mimetype)return t.article_thumbnail.url="http://www.videogular.com/styles/themes/default/latest/videogular.css";if("video/x-msvideo"==e.media_mimetype)return t.article_thumbnail.url="http://www.videogular.com/styles/themes/default/latest/videogular.css";if("video/x-ms-wmv"==e.media_mimetype)return t.article_thumbnail.url="http://www.videogular.com/styles/themes/default/latest/videogular.css";if("video/quicktime"==e.media_mimetype)return t.article_thumbnail.url="http://www.videogular.com/styles/themes/default/latest/videogular.css"}}}),e.tagscopy=angular.copy(e.tags),e.tagscopy=r.uniqBy(e.tagscopy,function(t){return t.tags}),e.tagscopy=r.filter(e.tagscopy,function(t){if("Article"!==t.tags&&"Portfolio"!==t.tags)return t}),e.lists=angular.copy(e.articlesList);var s=c("filter")(e.tags,{tags:"Portfolio"}),n=c("filter")(e.tags,{tags:"Article"});s&&r.each(s,function(t){var i=r.find(e.lists,{article_id:t.article_id});e.Portfolio.push(i),r.each(e.articlesList,function(e){e.article_id==t.article_id&&(e.article_type=t.tags)})}),n&&r.each(n,function(t){var i=r.find(e.lists,{article_id:t.article_id});e.Article.push(i),r.each(e.articlesList,function(e){e.article_id==t.article_id&&(e.article_type=t.tags)})})}})},e.product=function(){o.remove("label"),o.remove("hardtype"),l.go("main.productCatalogue")},e.toggleSelectedTag=function(t){e.event=["Filter by Industry","click",t.tags,r.toString(t.tags_id)],e.tagselected.indexOf(t)==-1?e.tagselected.push(t):e.tagselected=r.filter(e.tagselected,function(e){return e.tags_id!=t.tags_id})},e.trackExperienceArticles=function(t){d.trackEvent("Experience","View",t.title,r.toString(t.article_id))},e.trackExperiencePortfolio=function(t){d.trackEvent("Experience","View",t.title,r.toString(t.article_id))},e.trackExperienceAll=function(t){d.trackEvent(t.type,"View",t.title,r.toString(t.article_id))},e.isTagSelected=function(t){return e.tagselected.indexOf(t)!==-1},e.isOnSelectedTag=function(){return function(t){if(!(e.tagselected.length>0))return!0;var i=!1,a=r.map(e.tagselected,"tags_id");r.forEach(t.tags,function(t){a.indexOf(t.tags_id)!==-1&&(i=!0)});return i}},e.filterArticle=function(t){e.expFilter=t,o.set("articlefilter",JSON.stringify(e.expFilter))},e.$watch("expFilter",function(t){u(function(){if("all"==t)e.articlesList=angular.copy(e.lists);else{e.articlesList=[];var i=c("filter")(e.tags,{tags:t});r.each(i,function(t){var i=r.find(e.lists,{article_id:t.article_id});e.articlesList.push(i)})}},600)}),e.$on("$stateChangeSuccess",function(t,i,a,r,l){"main.experience"==i.name&&(e.tagselected=[],e.generateArticles())})}angular.module("tessaApp").service("imageService",["$q","$http",function(t,e){this.loadImages=function(){return e.jsonp("https://api.flickr.com/services/feeds/photos_public.gne?format=json&jsoncallback=JSON_CALLBACK")}}]).controller("expCtrl",t),t.$inject=["$http","$scope","APIFactory","$location","_","$state","$rootScope","$filter","$uibModal","localStorageService","$timeout","$frontierAnalytics","imageService","angularGridInstance"]}();
...
!function(){"use strict";function e(e,t,r,a,i,s,o,c,n,u){i.compareproducts=r.isEmpty(c.get("ProductsCompare"))?[]:JSON.parse(c.get("ProductsCompare"));e.hardwaresolutions=[],e.softwaresolutions=[],e.softwareshardattrib=[],e.softwaresscreenattrib=[],e.softwaresmediaattrib=[],e.compare={},e.comparelists=[],e.comparehardwarelists=[],e.compareattribute=[];r.each(i.compareproducts,function(t,a){if(0==a&&(e.compare=t),e.compare.p_id){var i={};i.title=t.item_name,i.attributes=t.attributes,i.image=t.img_default,i.price=t.sell_price,i.id=t.p_id,i.type=t.type,i._id=t._id,e.comparehardwarelists.push(i),e.hardwaresolutions.push(t.solution)}else if(t.hardware&&r.each(t.hardware,function(t){e.compareattribute.push(t)}),t.support&&t.support.length>0){var o=[],c=s("filter")(t.support,"Essential");r.each(c,function(e){o.push(e.product_detail.item_name)});var n=s("filter")(o,"Essential"),u=s("filter")(t.support,"Enterprise");r.each(t.support,function(e){r.each(n,function(r){r==e.product_detail.item_name&&(t.essential="Essential")}),r.each(u,function(r){e.product_detail.item_name==r.product_detail.item_name&&(t.enterprise="Enterprise")})})}t.tags&&t.tags.length>0}),e.softwaresolutions&&(e.softwaresolutions=r.flattenDeep(e.softwaresolutions)),e.compare.pb_id?async.waterfall([function(a){async.each(e.compareattribute,function(e,r){t.getProductAttribute(e.product_id,function(t,a){200==a.statusCode&&a.response.success&&200==a.statusCode&&a.response.success&&(e.atrribute=a.response.result.attribute_values,r())})},function(t,s){r.each(i.compareproducts,function(t){e.indi={},e.indi.titles=[];var a=[];t.hardware&&r.each(t.hardware,function(e){1==e.comparable&&a.push(e)});var i=r.groupBy(a,"category_label"),s=r.groupBy(t.software,"category_label");e.indi.hardwaregroup=i,e.indi.softwaregroup=s,e.indi.titles.title=t.bundle_name,e.indi.titles.image=t.product_images[0].image_path,e.indi.price=t.upfront,e.indi.id=t.pb_id,e.indi.weekly=t.weekly,e.indi.enterprise=t.enterprise,e.indi.essential=t.essential,e.indi._id=t._id,e.comparelists.push(e.indi)}),a(null,e.comparelists)})},function(t,r){e.comparelist=compare.getListGroupBy(t,e.groupBy,"-")}]):e.comparehardware=compare.getListGroupBy(e.comparehardwarelists,e.groupBy,"-"),e.removeItem=function(e){var t=r.findIndex(i.compareproducts,function(t){return t._id==e._id});i.compareproducts.splice(t,1),u.success("Successfully removed an item.","Success"),a.go(a.current,{},{reload:!0}),c.set("ProductsCompare",JSON.stringify(i.compareproducts))},e.toggle=!1,e.vewDetail=function(e){e.p_id?a.go("main.productDetailsHardware",{selected_id:e.p_id},{reload:!0}):e.pb_id&&a.go("main.productDetailsSolution",{selected_id:e.pb_id},{reload:!0})},e.comparisonShare=function(){var e=o.open({animation:!0,templateUrl:"./views/share.modal.html",size:"md",windowClass:"share-modal",controller:"ShareModalInstanceCtrl",resolve:{article:function(){return{}},param:function(){return"compare"}}});e.result.then(function(){},function(){console.info("Modal dismissed at: "+new Date)})},e.addToCart=function(e){if(i.cartItems.length>=0)if(e.quantity=1,e.pb_id){if(0==e.upfront)u.error("Cannot add product when the values are 0","Error");else if(i.cartItems.length>=0){var t=r.filter(i.cartItems,{pb_id:e.pb_id});t.length>0?(t[0].quantity++,r.each(i.cartItems,function(e){e.pb_id==t[0].pb_id&&(e.quantity=t[0].quantity)}),u.success("Quantity Added","Success"),c.set("Cart",JSON.stringify(i.cartItems))):(u.success("Successfully Added","Success"),i.cartItems.push(e),c.set("Cart",JSON.stringify(i.cartItems)))}}else{e.upfront=e.sell_price;var t=r.filter(i.cartItems,{p_id:e.p_id});t.length>0?(t[0].quantity++,r.each(i.cartItems,function(e){e.p_id==t[0].p_id&&(e.quantity=t[0].quantity)}),u.success("Quantity Added","Success"),c.set("Cart",JSON.stringify(i.cartItems))):(u.success("Successfully Added","Success"),i.cartItems.push(e),c.set("Cart",JSON.stringify(i.cartItems)))}}}angular.module("tessaApp").controller("compareCtrl",e),e.$inject=["$scope","APIFactory","_","$state","$rootScope","$filter","$uibModal","localStorageService","$timeout","toastr"]}();
