...
!function(){"use strict";angular.module("tessaApp").run(["$location","APIFactory","$rootScope","$window","$state","localStorageService","$frontierAnalytics","usSpinnerService","AuthFactory","jwtHelper","API_URL","EXT_ID","toastr","$filter","$timeout",function(e,t,r,n,i,o,s,a,c,u,l,p,d,g,h){r._=window._,window.API_URL=l,window.EXT_ID=p,r.compareproducts=[],r.allproducts=[],r.cartItems=[],r.cartItems=_.isEmpty(o.get("Cart"))?[]:JSON.parse(o.get("Cart")),r.compareproducts=_.isEmpty(o.get("ProductsCompare"))?[]:JSON.parse(o.get("ProductsCompare")),s.debugMode(!1),s.allowPageTracking(!0),s.allowEventTracking(!0),s.allowViewTracking(!0),s.allowAppTracking(!0),s.allowDeviceTracking(!0),s.Initialize({accessCode:"958441082755103",trackerName:"project-tessa",encoding:"utf-8"});var f=function(){async.waterfall([function(e){t.getArticles(function(t,r){_.each(r.response.result,function(e){e.type="Experience"}),_.each(r.response.result,function(e){e.name=e.title,e.id=e.article_id}),e(null,r.response.result)})},function(e,r){t.getProductBundle(function(t,n){_.each(n.response.result,function(e){e.type="Solutions"}),_.each(n.response.result,function(e){e.name=e.bundle_name,e.id=e.pb_id}),r(null,e,n.response.result)})},function(e,r,n){var i=[];t.getProduct(function(t,o){i=_.filter(o.response.result,function(e){return"Hardware"==e.product_type}),_.each(i,function(e){e.type="Hardware"}),_.each(i,function(e){e.name=e.item_name,e.id=e.p_id}),n(null,e,r,i)})},function(e,r,n,i){var o=[];async.each(n,function(e,r){t.getProductAttribute(e.p_id,function(t,n){if(200==n.statusCode&&n.response.success&&e.p_id==n.response.result.p_id){var i=n.response.result.attribute_values;i&&i.length>0?e.attributes=n.response.result.attribute_values:e.attributes=null,r()}})},function(t,s){_.each(n,function(e){1==e.is_published&&0==e.is_archived&&o.push(e)}),i(null,e,r,n,o)})},function(e,t,n,i,o){var s=[];_.each(i,function(e){_.isNull(e.attributes)||s.push(e.attributes)}),s=_.flattenDeep(s),r.Brightness=g("filter")(s,{attribute:"Brightness"}),r.Bezel=g("filter")(s,{attribute:"Bezel Size"}),r.Usage=g("filter")(s,{attribute:"Usage"}),r.Brightness=_.uniqBy(r.Brightness,"value"),r.Bezel=_.uniqBy(r.Bezel,"value"),r.Usage=_.uniqBy(r.Usage,"value");var a=[];_.each(i,function(e){1==e.is_published&&0==e.is_archived&&a.push(e)}),_.each(t,function(e){1==e.is_published&&0==e.is_archived&&a.push(e)}),_.each(e,function(e){1==e.is_active&&a.push(e)}),o(null,i,t,e),r.allproducts=angular.copy(a)}])};if(c.getCurrentUser()){var m=JSON.parse(o.get("appuser"));f(),s.setUserId(_.toString(m.uc_id))}r.$on("InitializeData",function(){f()}),r.$on("loading:progress",function(){a.spin("spinner-1")}),r.$on("loading:finish",function(){a.stop("spinner-1")}),r.$on("app:reload",function(){h(function(){n.location.reload()},1e3)}),r.goBack=function(){n.history.back()},c.getCurrentUser()&&f(),r.$on("$stateChangeStart",function(t,n,s,a,l){if(r.compareIs=!0,_.isNull(c.getCurrentUser()))"login"!==n.name&&"forget-password"!==n.name&&"publicshare"!==n.name&&(t.preventDefault(),i.go("login"),r.$broadcast("app:reload"));else{var p=c.getUserToken();JSON.parse(o.get("appuser"));u.isTokenExpired(p)?c.logout().then(function(){r.$broadcast("app:reload"),e.path("/login")}):"login"==n.name&&(t.preventDefault(),i.go("main.experience"))}r.isHide=!1,r.toggleShow=function(){r.isHide=!r.isHide},r.width=Math.max(document.documentElement.clientWidth,window.innerWidth||0),r.height=Math.max(document.documentElement.clientHeight,window.innerHeight||0),console.log(" $rootScope.width:",r.width),console.log(" $rootScope.height:",r.height),r.hideNav=function(){r.isHide=!1},r.compare=function(e){var t={},n=0;if(n=r.width<=500&&r.height<=975?2:r.width<=975&&r.height<=500?2:4,_.isEmpty(r.compareproducts))d.success("Successfully Added","Success"),r.compareIs=!1,r.compareproducts.push(e),o.set("ProductsCompare",JSON.stringify(r.compareproducts));else if(r.compareproducts.length>=1&&r.compareproducts.length<n){var i=_.filter(r.compareproducts,{_id:e._id});i.length>0?d.error("Oops. The selected item has already been added.","Warning"):(_.each(r.compareproducts,function(e,r){0==r&&(t=e)}),t.category_id==e.category_id?(d.success("Successfully Added","Success"),r.compareIs=!1,r.compareproducts.push(e),o.set("ProductsCompare",JSON.stringify(r.compareproducts))):d.error("Oops. The selected item is incomparable to the existing item(s).","Error"))}else d.error("Oops. You can not compare more than "+n+" items.","Error")}}),r.$on("$stateChangeSuccess",function(e,t,n,i,o){r.appuser=JSON.parse(c.getCurrentUser("appuser")),r.currentUser=c.getCurrentUser(),r.label="Solutions","main.search"==t.name&&f(),s.trackPage(t.name,t.url,t.templateUrl)})}])}();
...
!function(){"use strict";function e(e,t,r,a,s,i,o,c,n,u){s.compareproducts=r.isEmpty(c.get("ProductsCompare"))?[]:JSON.parse(c.get("ProductsCompare"));e.hardwaresolutions=[],e.softwaresolutions=[],e.softwareshardattrib=[],e.softwaresscreenattrib=[],e.softwaresmediaattrib=[],e.compare={},e.comparelists=[],e.comparehardwarelists=[],e.compareattribute=[];console.log("$rootScope.compareproducts:",s.compareproducts),r.each(s.compareproducts,function(t,a){if(0==a&&(e.compare=t),e.compare.p_id){var s={};s.title=t.item_name,s.attributes=t.attributes,s.image=t.img_default,s.price=t.sell_price,s.id=t.p_id,s.type=t.type,s._id=t._id,e.comparehardwarelists.push(s),e.hardwaresolutions.push(t.solution)}else if(t.hardware&&r.each(t.hardware,function(t){e.compareattribute.push(t)}),t.support&&t.support.length>0){var o=[],c=i("filter")(t.support,"Essential");r.each(c,function(e){o.push(e.product_detail.item_name)});var n=i("filter")(o,"Essential"),u=i("filter")(t.support,"Enterprise");r.each(t.support,function(e){r.each(n,function(r){r==e.product_detail.item_name&&(t.essential="Essential")}),r.each(u,function(r){e.product_detail.item_name==r.product_detail.item_name&&(t.enterprise="Enterprise")})})}t.tags&&t.tags.length>0}),e.softwaresolutions&&(e.softwaresolutions=r.flattenDeep(e.softwaresolutions)),e.compare.pb_id?async.waterfall([function(a){async.each(e.compareattribute,function(e,r){t.getProductAttribute(e.product_id,function(t,a){200==a.statusCode&&a.response.success&&200==a.statusCode&&a.response.success&&(e.atrribute=a.response.result.attribute_values,r())})},function(t,i){r.each(s.compareproducts,function(t){e.indi={},e.indi.titles=[];var a=[];t.hardware&&r.each(t.hardware,function(e){1==e.comparable&&a.push(e)});var s=r.groupBy(a,"category_label"),i=r.groupBy(t.software,"category_label");e.indi.hardwaregroup=s,e.indi.softwaregroup=i,e.indi.titles.title=t.bundle_name,e.indi.titles.image=t.product_images[0].image_path,e.indi.price=t.upfront,e.indi.id=t.pb_id,e.indi.weekly=t.weekly,e.indi.enterprise=t.enterprise,e.indi.essential=t.essential,e.indi._id=t._id,e.comparelists.push(e.indi)}),console.log("$scope.comparelists:",e.comparelists),a(null,e.comparelists)})},function(t,r){e.comparelist=compare.getListGroupBy(t,e.groupBy,"-")}]):e.comparehardware=compare.getListGroupBy(e.comparehardwarelists,e.groupBy,"-"),e.removeItem=function(e){var t=r.findIndex(s.compareproducts,function(t){return t._id==e._id});s.compareproducts.splice(t,1),u.success("Successfully removed an item.","Success"),a.go(a.current,{},{reload:!0}),c.set("ProductsCompare",JSON.stringify(s.compareproducts))},e.toggle=!1,e.vewDetail=function(e){e.p_id?a.go("main.productDetailsHardware",{selected_id:e.p_id},{reload:!0}):e.pb_id&&a.go("main.productDetailsSolution",{selected_id:e.pb_id},{reload:!0})},e.comparisonShare=function(){var e=o.open({animation:!0,templateUrl:"./views/share.modal.html",size:"md",windowClass:"share-modal",controller:"ShareModalInstanceCtrl",resolve:{article:function(){return{}},param:function(){return"compare"}}});e.result.then(function(){},function(){console.info("Modal dismissed at: "+new Date)})},e.addToCart=function(e){if(s.cartItems.length>=0)if(e.quantity=1,e.pb_id){if(0==e.upfront)u.error("Cannot add product when the values are 0","Error");else if(s.cartItems.length>=0){var t=r.filter(s.cartItems,{pb_id:e.pb_id});t.length>0?(t[0].quantity++,r.each(s.cartItems,function(e){e.pb_id==t[0].pb_id&&(e.quantity=t[0].quantity)}),u.success("Quantity Added","Success"),c.set("Cart",JSON.stringify(s.cartItems))):(u.success("Successfully Added","Success"),s.cartItems.push(e),c.set("Cart",JSON.stringify(s.cartItems)))}}else{e.upfront=e.sell_price;var t=r.filter(s.cartItems,{p_id:e.p_id});t.length>0?(t[0].quantity++,r.each(s.cartItems,function(e){e.p_id==t[0].p_id&&(e.quantity=t[0].quantity)}),u.success("Quantity Added","Success"),c.set("Cart",JSON.stringify(s.cartItems))):(u.success("Successfully Added","Success"),s.cartItems.push(e),c.set("Cart",JSON.stringify(s.cartItems)))}}}angular.module("tessaApp").controller("compareCtrl",e),e.$inject=["$scope","APIFactory","_","$state","$rootScope","$filter","$uibModal","localStorageService","$timeout","toastr"]}();
